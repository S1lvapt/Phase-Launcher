...
export function BigFeaturedCard({ item }: { item: ShopItem }) {
  ...
  return (
    <motion.div
      ...
      className="relative w-61 h-105.25 rounded-lg overflow-hidden shadow-xl border border-stone-800 bg-black"
    >
      <div
      ...
      <AnimatePresence mode="wait">
        <motion.img
          ...
          className="absolute inset-0 m-auto object-cover w-full h-full max-w-full max-h-full drop-shadow-2xl"
          draggable={false}
          onError={(e) => {
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/95 via-black/80 to-transparent px-5 py-4">
        <h3 className="text-white text-lg font-extrabold leading-tight line-clamp-1">
          {item.name}
        </h3>
